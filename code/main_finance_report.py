...
import sys
from sales_pipeline import get_raw_sales_data, clean_sales_data, calculate_total_revenue, print_sales_table
...
# --- The report ------------------------------------------------------------------
#
# Fill in each TDO below. This first report names the exact function to call and
...
# TDO: import what this report needs from the package. Four names, all of them
...
# TOD: print the header, exactly:   === FINANCE: Daily Sales Detail ===
...
#    TDO: call get_raw_sales_data(seed) and store the result in `raw_data`.
raw_data = get_raw_sales_data(seed)

# 2. Transform — clean it, then total it.
#    TOD: call clean_sales_data(raw_data) and store it in `clean_data`.
#    TOD: call calculate_total_revenue(clean_data) and store it in `total_revenue`.
clean_data = clean_sales_data(raw_data)
total_revenue = calculate_total_revenue(clean_data)

# 3. Load — put it in front of a human.
#    TDO: call print_sales_table(clean_data).
#    TDO: print() a blank line.
#    TDO: print the total. Use an f-string with the same format spec display.py
